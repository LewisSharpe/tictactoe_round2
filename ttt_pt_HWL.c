...
/* a data-structure that is used as sole input argument to a function that is c$ */
typedef struct {
  int *arr;
  int len;
  int *res;
	...
	// MAIN LOOP	
	  ...
	  for (t=0; t<NUM_THREADS && index < MoveCount; t++, index++) {
	    ...
	    //		*(thr_data+49*sizeof(int)) = side^1;
            ...
            rc[t] = pthread_create(&thr[i], NULL, MinMax, (void *) /* new_thread_arg or: */ thread_args[t] ); // HWL
#endif
	    // undo moves
	    board0[Move] = EMPTY; // else clear board; no longer needed if you use new_thread_arg -- HWL
	    ply--; // decrement ply
	  }
	  ...
	  for (t = 0; t < NUM_THREADS; ++t) {
	    ...
	    score = *(thread_args[t]->res); // use this to pickup result in pthread version -- HWL
